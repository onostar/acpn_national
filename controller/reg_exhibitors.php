<?php
    ...
    if(isset($_POST['register_exhibitor'])){
        ...
        $manager_contact = ucwords(htmlspecialchars(stripslashes($_POST['manager_contact'])));
        ...
        if(strlen($password) < 5){
            ...
            if($check_user->rowCount() > 0){
                ...
                if(move_uploaded_file($_FILES['company_logo']['tmp_name'], $logo_folder)){
                    $insert_user = $connectdb->prepare("INSERT INTO exhibitors (company_name, company_address, company_phone, contact_person, designation, contact_phone, company_email, staff_number, portal_manager, manager_contact, company_password, company_logo, reg_number) VALUES (:company_name, :company_address, :company_phone, :contact_person, :designation, :contact_phone, :company_email, :staff_number, :portal_manager, :manager_contact, :company_password, :company_logo, :reg_number)");
                    ...
                    $insert_user->bindvalue("portal_manager", $portal_manager);
                    $insert_user->bindvalue("manager_contact", $manager_contact);
                    ...
                    if($insert_user){
                        ...
                        /* update reg_num */
                        $update_reg = $connectdb->prepare("UPDATE exhibitors SET reg_number = :reg_number WHERE company_email = :company_email");
                        $update_reg->bindvalue("reg_number", $new_reg);
                        $update_reg->bindvalue("company_email", $email);
                        $update_reg->execute();
                        /* update payment status */
                        
                        $_SESSION['reg_success'] = "Your registration was successful, Kindly view our floor plan and select a booth to continue!";
